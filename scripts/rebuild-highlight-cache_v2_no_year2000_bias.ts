...
function scoreYear(year: number, state: State) {
  const yearCount = state.yearUsage.get(year) ?? 0;
  const decadeCount = state.decadeUsage.get(getDecade(year)) ?? 0;
  const mildYear2000Penalty = year === 2000 ? 0.35 : 0;

  return yearCount * 100 + decadeCount * 10 + mildYear2000Penalty;
}
...
async function seedOnePassForAllMonthDays(state: State) {
  console.log("Seeding one pass for all month-days with balanced years...");

  const templates = buildMonthDayTemplates();

  for (const [index, template] of templates.entries()) {
    const candidateYears = shuffleArray(
      getCandidateYearsForMonthDay(template.month, template.day)
    ).sort((a, b) => scoreYear(a, state) - scoreYear(b, state));

    let success = false;

    for (const year of candidateYears.slice(0, 60)) {
...
async function backfillMissingOrWeakMonths(state: State) {
  ...
  while (true) {
    ...
    const candidateYears = shuffleArray(
      getCandidateYearsForMonthDay(target.month, target.day)
    ).sort((a, b) => scoreYear(a, state) - scoreYear(b, state));

    let success = false;

    for (const year of candidateYears.slice(0, 80)) {
...
async function main() {
  ...
  const topYears = [...state.yearUsage.entries()]
    .sort((a, b) => b[1] - a[1])
    .slice(0, 20)
