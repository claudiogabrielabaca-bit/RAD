...
import { readPickDateHighlight } from "@/app/lib/pick-date-cache";
...
export async function buildPickDateBundle(day: string) {
  const [user, highlightResult, ratings, stats]: [
    CurrentUser,
    Awaited<ReturnType<typeof readPickDateHighlight>>,
    RatingRecord[],
    { views: number } | null,
  ] = await Promise.all([
    getCurrentUser(),
    readPickDateHighlight(day),
  ...
  const avg =
    count === 0
      ? 0
      : ratings.reduce((acc: number, r: RatingRecord) => acc + r.stars, 0) /
        count;
