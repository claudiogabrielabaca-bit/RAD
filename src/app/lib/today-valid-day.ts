...
export async function getTodayValidDay(options?: {
  ...
  /*
   * Important:
   * TodayHistoryPool.validDays is already the persisted Today pool.
   * Do NOT throw it away just because those days are not currently present
   * in DayHighlightCache. That was the bug that made an 83-day pool behave
   * like a ~35-day pool.
   */
   ...
   * - tested in DayHighlightCache
   * - not usable there
   *
   * Keep stored TodayHistoryPool days that are not in DayHighlightCache,
   * because they may have been generated by the Today pool builder and are
   * valid even if the current highlight cache table does not contain them.
   */
  ...
  /*
   * Do not regenerate days that are already in TodayHistoryPool, even if they
   * are not in DayHighlightCache. They are part of the stored pool.
   */
  ...
  /*
   * In fresh mode we prefer a newly generated valid day if one was found,
   * but we still keep the stored pool as fallback.
   */
  ...
  /*
   * If all stored pool days were excluded, restart the local round using the
   * full stored Today pool, not only DayHighlightCache-confirmed rows.
   */
